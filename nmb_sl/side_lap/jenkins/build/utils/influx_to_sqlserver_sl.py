...
class SIDELAP(PREPARE):
    ...
    def query_influx(self) :
        try:
            ...
            for i in range(len(mqtt_topic_value)):
                query = f"select time,topic,d_str1,d_str2,rssi,counter_1,status_1,counter_2,status_2,counter_3,status_3 from mqtt_consumer where topic ='{mqtt_topic_value[i]}' order by time desc limit 10"
    ...
    def edit_col(self):
            try:
                ...
                for i in range(1, 4):
                    ...
                    counter_col = f'counter_{i}'
                    df.loc[df[status_col] > 0, 'counter_no'] = i
                    df.loc[df[status_col] > 0, 'value'] = df[counter_col]

                self.df_influx = df[['occurred', 'd_str1', 'd_str2', 'counter_no', 'value','rssi', 'mc_no', 'process']]    
    ...
    def check_duplicate(self):
        try:
            ...
            init_list = ['process','d_str1','d_str2']
    ...
    def df_to_db(self):
        #connect to db
        init_list = ['occurred','mc_no','process','d_str1','d_str2']
        insert_db_value = self.column_names.split(",")
        col_list = init_list + insert_db_value
